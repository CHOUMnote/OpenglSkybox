...
class MyLitGround(val myContext : Context) {
    ...
    private val color = floatArrayOf(.8f, .8f, .8f, 1f)
    private val lightDir = floatArrayOf(1.0f,1.0f,1.0f)
    private val lightAmbient = floatArrayOf(0f,0f,0f)
    private val lightDiffuse = floatArrayOf(1f,1f,1f)
    private val lightSpecular = floatArrayOf(1.0f,1.0f,1.0f)
    private val matAmbient = floatArrayOf(1f,1f,1f)
    ...
    fun draw(mvpMatrix:FloatArray, worldMat:FloatArray){
        ...
        GLES30.glUniform3fv(mLightDirHandle, 1, com.example.skyoxmodel.lightDir, 0)
